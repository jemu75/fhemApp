      ...
      <v-card-text>
        <v-row align="center">
          ...
          <v-col align="center" v-if="active">
            <div class="headline font-weight-bold">{{ vals.level }}</div>
          </v-col>
        </v-row>
      </v-card-text>
      <v-divider></v-divider>
  ...
  export default {
    data: () => ({
      ...
      defaultSet: [
        ...
        "level:5::level:success:mdi-water",
        "level:::level:error:mdi-water-off"
      ],
      vals: {
        order: '',
        title: '',
    ...
    watch: {
      item: {
        ...
        deep: true,
        handler(val) {
          let liter = this.$fhem.getEl(val, 'Readings', 'liter', 'Value');
          let level = this.$fhem.getEl(val, 'Readings', 'level', 'Value');
