...
<script>
  export default {
    ...
    methods: {
      clickStart(val) {
        ...
        this.timer = setInterval(() => {
          ...
          if(action) {
            let param = this.$fhem.checkVal(this.item, action);
            if(param[0]) this.$fhem.request('set ' + this.item.Name + ' ' + param[0]);
          }
        }, 1000)
      },
      ...
      setLevel(lvl) {
        ...
        let mainText = this.$fhem.checkVal(this.item, this.setup.main[this.mainLevel].text);
        this.vals.main.text = mainText[0] || '';
      },

      setValues() {
        if(this.setup.main.length > 0) {
          let statusVals = this.$fhem.checkVal(this.item, this.setup.status.line);
          let errorVals = this.$fhem.checkVal(this.item, this.setup.status.error);
          let mainText = this.$fhem.checkVal(this.item, this.setup.main[this.mainLevel].text);
          let infoLeft1Vals = this.$fhem.checkVal(this.item, this.setup.info.left1);
          let infoLeft2Vals = this.$fhem.checkVal(this.item, this.setup.info.left2);
          let infoMid1Vals = this.$fhem.checkVal(this.item, this.setup.info.mid1);
          let infoMid2Vals = this.$fhem.checkVal(this.item, this.setup.info.mid2);
          let infoRight1Vals = this.$fhem.checkVal(this.item, this.setup.info.right1);
          let infoRight2Vals = this.$fhem.checkVal(this.item, this.setup.info.right2);
    ...
    created() {
      let name = this.$fhem.getEl(this.item, 'Options', 'customdef');
      let idx = this.$fhem.app.custom.map((e) => e.name).indexOf(name);
      let cfg = idx != -1 ? this.$fhem.app.custom[idx] : {};
