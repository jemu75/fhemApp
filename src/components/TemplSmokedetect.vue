...
<script>
  export default {
    data: () => ({
      name: 'smokedetect',
      defaultSet: [
        "Activity:^(?!alive):keine Verbindung:100:error:mdi-smoke-detector",
        "level:2:Alarm:100:error:mdi-fire",
        "level::bereit:100:success:mdi-smoke-detector"
      ],
      vals: {
        order: '',
        title: '',
    ...
    watch: {
      item: {
        ...
        deep: true,
        handler(val) {
          let lastAlert = this.$fhem.getEl(val, 'Readings', 'trigger_cnt', 'Time');
          ...
          this.vals.systemIconValue = val.Readings.level.Value != '1' ? val.Readings.level.Value : '';

          this.vals.systemLastAlert = lastAlert ? this.$fhem.getDateTime(lastAlert) : '';
    ...
    methods: {
      set() {
        let deviceName = this.item.Readings.peerList.Value.split(',')[0];
        if(deviceName.match('self01')) deviceName = this.item.Name;

        let cmd = 'set ' + deviceName + ' alarmOff';
