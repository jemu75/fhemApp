      ...
      <v-card-text>
        <v-row align="center">
          ...
          <v-col align="center" v-if="active">
            <div class="headline font-weight-bold">{{ vals.humidity }}</div>
          </v-col>
        </v-row>
      </v-card-text>
      <v-divider></v-divider>
...
<script>
  export default {
    data: () => ({
      name: 'thermometer',
      defaultSet: [
        "Activity:^(?!alive):keine Verbindung:100:error",
        "temperature:0::0:success:mdi-thermometer",
        "temperature:::0:success:mdi-snowflake"
      ],
      vals: {
        order: '',
        title: '',
